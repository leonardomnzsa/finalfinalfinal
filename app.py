...
openai_api_key = os.getenv("OPENAI_API_KEY") # Get key from environment variable
if not openai_api_key:
    st.error("Chave da API OpenAI (OPENAI_API_KEY) não configurada nas variáveis de ambiente do Render. As funcionalidades RESULT não funcionarão.")
    # raise Exception("OPENAI_API_KEY não configurada.") # Keep this commented out for now
else:
...
PERSISTENT_DATA_PATH = Path("/mnt/data")
PERSISTENT_DATA_PATH.mkdir(parents=True, exist_ok=True) # Ensure main persistent dir exists
USER_CREDENTIALS_FILE = PERSISTENT_DATA_PATH / "user_credentials.json"
USER_DATA_DIR = PERSISTENT_DATA_PATH / "user_data"
LOG_FILE = PERSISTENT_DATA_PATH / "activity_log.csv" # Also making log persistent
USER_DATA_DIR.mkdir(parents=True, exist_ok=True) # Ensure user data subdir exists
...
if not st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        if df_informativos_exploded is not None:
            ...
            selected_anos = []
            selected_meses_anos = []
# The lines defining selected_ramos, selected_rg etc. were removed from here
# as their placement was incorrect. Filter defaults should be handled within the sidebar logic.
